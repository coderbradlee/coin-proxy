...
type Client interface {
	CreateAddress(account_index string) (addr string, err error)
	// Return the wallet's balance.
	GetBalance(account_index string) (balance, unlockedBalance uint64, err error)
...
func (c *client) GetBalance(account_index string) (balance, unlockedBalance uint64, err error) {
	jd := struct {
		Balance         uint64 `json:"balance"`
		UnlockedBalance uint64 `json:"unlocked_balance"`
	}{}
	accindex, err := strconv.ParseInt(account_index, 10, 64)
	if err != nil {
		return
	}
	from := struct {
	...
	err = c.do("getbalance", from, &jd)
	return jd.Balance, jd.UnlockedBalance, err
}
func (c *client) CreateAddress(account_index string) (addr string, err error) {
	jd := struct {
		Address       string `json:"address"`
		Address_index uint64 `json:"address_index"`
	}{}
	accindex, err := strconv.ParseInt(account_index, 10, 64)
	if err != nil {
		return
	...
	err = c.do("create_address", from, &jd)
	addr = jd.Address
	return
}
