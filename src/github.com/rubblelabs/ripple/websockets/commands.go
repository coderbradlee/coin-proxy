...
type AccountTxCommand2 struct {
	*Command
	...
	Limit     int                    `json:"limit,omitempty"`
	Marker    map[string]interface{} `json:"marker,omitempty"`
	Result    *AccountTxResult2      `json:"result,omitempty"`
}

type AccountTxResult2 struct {
	Marker       map[string]interface{}           `json:"marker,omitempty"`
	Transactions []*data.TransactionWithMetaData2 `json:"transactions,omitempty"`
}
...
func newAccountTxCommand2(account data.Account, pageSize int, marker map[string]interface{}, minLedger, maxLedger int64) *AccountTxCommand2 {
	return &AccountTxCommand2{
		Command:   newCommand("account_tx"),
		Account:   account,
		MinLedger: minLedger,
		MaxLedger: maxLedger,
