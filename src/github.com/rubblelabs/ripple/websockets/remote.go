...
func (r *Remote) accountTx2(account data.Account, c chan *data.TransactionWithMetaData2, pageSize int, minLedger, maxLedger int64) {
	defer close(c)
	cmd := newAccountTxCommand2(account, pageSize, nil, minLedger, maxLedger)
	for ; ; cmd = newAccountTxCommand2(account, pageSize, cmd.Result.Marker, minLedger, maxLedger) {
		r.outgoing <- cmd
		<-cmd.Ready
		if cmd.CommandError != nil {
			glog.Errorln(cmd.Error())
		...
		fmt.Println("xxxx:", cmd.Result.Marker)
		if cmd.Result.Marker == nil {
			return
		}
	}
...
func (r *Remote) AccountTx2(account data.Account, pageSize int, minLedger, maxLedger int64) chan *data.TransactionWithMetaData2 {
	c := make(chan *data.TransactionWithMetaData2)
	go r.accountTx2(account, c, pageSize, minLedger, maxLedger)
	return c
}
