...
type TrxIn struct {
	...
	ContextFreeActions []*Action2      `json:"context_free_actions"`
...
type Trx struct {
	Receipt struct {
		...
		CPUUsageMicrosec int               `json:"cpu_usage_us"`
		NetUsageWords    int               `json:"net_usage_words"`
		TrxInReceipt     []interface{}     `json:"trx"`
	} `json:"receipt"`
	TrxInTrx TrxIn `json:"trx"`
}
type TransactionResp2 struct {
	ID                    SHA256Bytes    `json:"id"`
	Trans                 Trx            `json:"trx"`
	BlockTime             JSONTime       `json:"block_time"`
	BlockNum              uint32         `json:"block_num"`
	LastIrreversibleBlock uint32         `json:"last_irreversible_block"`
	Traces                []ActionTrace2 `json:"traces"`
}
type ProcessedTransaction struct {
	Transaction SignedTransaction `json:"trx"`
}
type ActionTrace2 struct {
	Receipt struct {
	...
	Action             *Action2       `json:"act"`
	Context_free       bool           `json:"context_free"`
	...
	BlockNum           uint32         `json:"block_num"`
	BlockTime          JSONTime       `json:"block_time"`
	Producer_block_id  string         `json:"producer_block_id"`
	Account_ram_deltas []interface{}  `json:"account_ram_deltas"`
	Inline_traces      []ActionTrace2 `json:"inline_traces"`
}
