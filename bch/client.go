...
func (c *Client) GetTransaction(transactionId string) ([]btcjson.GetTransactionDetailsResult, error) {
	// btcjson.NewListReceivedByAddressCmd(btcjson.Int(6), nil, nil)
	// cmd, err := btcjson.NewCmd("sendtoaddress", to, f)
	// cmd, err := btcjson.NewCmd("sendfrom", fromAccount, to, f)
...
func (c *Client) Send(fromAccount, to, amount, fee, walletPass, topassword, dir string) (hash string, errs error) {
	{
		////首先验证密码
		b, err := ioutil.ReadFile(dir + "/" + to)
		if err != nil {
			// fmt.Print(err)
...
// OmniGetNewAddress ...
func (c *Client) GetNewAddress(account, password, dir, walletPass string) (string, error) {
	...
	err = writeFile(dir, walletPass, result, password)
	if err != nil {
		return "", err
	}
	return result, nil
...
func (c *Client) GetBalance(account string) (float64, error) {
	...
	// {"result":"1LXdpMTAQbUvV3EStdCb36Py6mMseYYXfd","error":null,"id":"curltext"}
	log.Println("getbalance:", string(resBytes))
	var result float64
	err = json.Unmarshal(resBytes, &result)
	if err != nil {
		return 0, err
...
func (c *Client) sendPostRequest(httpReq *http.Request, jReq *jsonRequest) ([]byte, error) {
	...
	log.Println("sendPostRequest:", string(respBytes))
	var res Response
