...
func dealwithLtcMethod(t rpcclient.JsonRequest, response *rpcclient.Response2, retError *rpcclient.ErrorStruct) {
	s, err := ltc.New(cfg.Ltc.Host, cfg.Ltc.Port, cfg.Ltc.Username, cfg.Ltc.Password)
	if err != nil {
	...
	case "send":
		...
		log.Println("fee:", fee)
		h, err := s.Send(from, to, amount, fee, cfg.Ltc.WalletPass)
		log.Println("hash:", h)
		if err != nil {
			retError.Message = err.Error()
			response.Error = retError
...
func LtcHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method != "POST" {
		...
		if err_decode != nil {
			...
			dealwithLtcMethod(t, &response, &retError)
		}
