...
func dealwithXmrMethod(t rpcclient.JsonRequest, response *rpcclient.Response2, retError *rpcclient.ErrorStruct) {

	s := xmr.NewRPCClient("xmr", cfg.Xmr.Url, cfg.Xmr.Username, cfg.Xmr.Password, "10s")
	log.Println(cfg.Xmr)
	switch t.Method {
	case "getbalance":
		var account string
...
func XmrHandler(w http.ResponseWriter, r *http.Request) {
	////////////////////////////////
	...
	if r.Method != "POST" {
		...
		if err_decode != nil {
			...
			dealwithXmrMethod(t, &response, &retError)
		}
