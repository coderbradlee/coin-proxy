...
func dealwithBchsvMethod(t rpcclient.JsonRequest, response *rpcclient.Response2, retError *rpcclient.ErrorStruct) {
	...
	s, err := bch.New(cfg.Bchsv.Host, cfg.Bchsv.Port, cfg.Bchsv.Username, cfg.Bchsv.Password)
	if err != nil {
	...
	case "getaddress":
		...
		// change, points, balance, err := s.GetBalance(addr)
		b, err := s.GetNewAddress(account, password, bchsv_dir, cfg.Bchsv.WalletPass)
	...
	case "send":
		...
		log.Println("fee:", fee)
		h, err := s.Send(from, to, amount, fee, cfg.Bchsv.WalletPass, password, bchsv_dir)
		log.Println("hash:", h)
		if err != nil {
			retError.Message = err.Error()
			response.Error = retError
...
func BchsvHandler(w http.ResponseWriter, r *http.Request) {
	////////////////////////////////
	...
	if r.Method != "POST" {
		...
		if err_decode != nil {
			...
			dealwithBchsvMethod(t, &response, &retError)
		}
