...
func dealwithBchabcMethod(t rpcclient.JsonRequest, response *rpcclient.Response2, retError *rpcclient.ErrorStruct) {
	...
	s, err := bch.New(cfg.Bchabc.Host, cfg.Bchabc.Port, cfg.Bchabc.Username, cfg.Bchabc.Password)
	if err != nil {
	...
	case "getaddress":
		...
		// change, points, balance, err := s.GetBalance(addr)
		b, err := s.GetNewAddress(account, password, bchabc_dir, cfg.Bchabc.WalletPass)
	...
	case "send":
		...
		if len(t.Params) >= 4 {
			...
			password = fmt.Sprintf("%v", t.Params[4])
		}
		log.Println("from:", from)
		log.Println("to:", to)
		log.Println("amount:", amount)
		log.Println("fee:", fee)
		h, err := s.Send(from, to, amount, fee, cfg.Bchabc.WalletPass, password, bchabc_dir)
		log.Println("hash:", h)
		if err != nil {
			retError.Message = err.Error()
			response.Error = retError
...
func BchabcHandler(w http.ResponseWriter, r *http.Request) {
	////////////////////////////////
	...
	if r.Method != "POST" {
		...
		if err_decode != nil {
			...
			dealwithBchabcMethod(t, &response, &retError)
		}
